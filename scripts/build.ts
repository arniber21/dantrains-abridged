...
import { validateTrain, validateGradeCrossingPreset } from "./trainSchema";
...
function serializeGradeCrossingPreset(name: string, preset: GradeCrossingPreset, level: number): string {
	...
	return [
		...
		`${indent(level + 1)}gradeCrossingMaintenancePerDay: ${formatNumber(preset.gradeCrossingMaintenancePerDay)},`,
...
/**
 * Refuses to emit a roster the game would mishandle.
 *
 * A missing stat is not rejected at runtime -- it silently becomes NaN cost or
 * broken physics -- so the build is the last place it can be caught cheaply.
 */
function checkSchema(
	data: TrainsYaml,
	presets: { heavyRail: GradeCrossingPreset; lightRail: GradeCrossingPreset }
): void {
	const problems: string[] = [];

	for (const train of data.newTrainTypes) {
		problems.push(...validateTrain(`newTrainTypes[${train.id ?? "?"}]`, train));
	}
	for (const { baseId, definition } of data.modifiedTrainTypes ?? []) {
		problems.push(...validateTrain(`modifiedTrainTypes[${baseId}]`, definition));
	}
	for (const [name, preset] of Object.entries(presets)) {
		problems.push(...validateGradeCrossingPreset(`gradeCrossingPresets.${name}`, preset as unknown as Record<string, unknown>));
	}

	if (problems.length > 0) {
		throw new Error(
			`trains.yaml does not match the train API schema:\n` +
				problems.map((p) => `  - ${p}`).join("\n") +
				`\n\nSee scripts/trainSchema.ts for the field list.`
		);
	}
}
...
function build(): void {
	...
	const modifiedTrainTypes = data.modifiedTrainTypes ?? [];

	checkSchema(data, gradeCrossingPresets);
	...
	out.push("");
	out.push("(function () {");
	out.push(`${TAB}const TAG = "[BasedGoat Trains]";`);
	out.push("");
	...
	for (const { baseId, definition } of modifiedTrainTypes) {
		out.push(`${TAB}${TAB}{`);
